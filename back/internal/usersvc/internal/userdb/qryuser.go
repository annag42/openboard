...
func (s *UserDB) findRoles(ctx cx, x *pb.FndRolesReq, y *pb.RolesResp) error {
	...
	// 	SELECT role_id, role_name
	// 	FROM role
	// 	WHERE role_id IN (?) LIMIT ? OFFSET ?;
	// `

	query, args, err := sqlx.In(qry, x.RoleIds, x.RoleNames, x.Limit, x.Lapse)
	//query, args, err := sqlx.In(qry, x.RoleIds, x.Limit, x.Lapse)
	if err != nil {
		return err
	}
	...
	// query, args, err = sqlx.In(qry, x.RoleIds, x.RoleNames)
	// if err != nil {
	// 	return err
	// }
	// query = s.db.Rebind(query)
	...
	qry = "SELECT COUNT(*) FROM role WHERE role_id IN (?)"
	query, args, err = sqlx.In(qry, x.RoleIds)
	if err != nil {
		return err
	}
	query = s.db.Rebind(query)
